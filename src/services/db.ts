...
class IndexedDBService<T extends { id?: IDBValidKey }> {
	...
	create(item: T): Promise<T & { id: IDBValidKey }> {
		return new Promise((resolve, reject) => {
			(async () => {
				try {
	...
	update(id: IDBValidKey, updatedItem: Partial<T>): Promise<void> {
		return new Promise((resolve, reject) => {
			(async () => {
				try {
					const store = await this.getStore('readwrite')
	...
	getAll(): Promise<Array<T & { id: IDBValidKey }>> {
		return new Promise((resolve, reject) => {
			(async () => {
				try {
	...
	getById(id: IDBValidKey): Promise<T & { id: IDBValidKey }> {
		return new Promise((resolve, reject) => {
			(async () => {
				try {
					const store = await this.getStore('readonly')
	...
	search(query: string, page: number = 1, limit: number = 10): Promise<Array<T & { id: IDBValidKey }>> {
		return new Promise((resolve, reject) => {
			(async () => {
				try {
					const store = await this.getStore('readonly')
					const request = store.openCursor()
					const results: Array<T & { id: IDBValidKey }> = []
					let skipped = 0
